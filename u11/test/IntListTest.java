...
import lists.CircularLinkedIntList;
...
//        }while(cur!=null&&cur!=null);
	...
	private void assertConsistency(IntList ll) {
		...
		assertEquals(list.getLast() == null, list.isEmpty());
		assertEquals(list.getLast() == null, list.getSize() == 0);
		

		if (!ll.isEmpty()) {
			assertNotEquals(null, list.getLast());
			
			IntNode n = list.getLast().getNext();
			IntNode m = null;
			for (int i = 0; i < list.getSize(); i += 1) {
				assertNotEquals(null, n.getNext());
				m = n;
				n = n.getNext();
			}
			
			assertEquals(list.getLast(), m);
		}
	}
